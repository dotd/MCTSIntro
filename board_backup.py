...
class Board(object):
    ...
    inv_positions = dict(
        (v, P) for P, v in positions
    )
    print(inv_positions)

    wins = []
    for r in range(3):
        wins.append(positions[(r, 0)] | positions[(r, 1)] | positions[(r, 2)])
    for c in range(3):
        wins.append(positions[(0, c)] | positions[(1, c)] | positions[(2, c)])
    wins.append(positions[(0, 0)] | positions[(1, 1)] | positions[(2, 2)])
    wins.append(positions[(0, 2)] | positions[(1, 1)] | positions[(2, 0)])
    print(wins)
    '''
    wins = [
